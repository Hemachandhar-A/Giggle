# GigShield — Agent Context & Specification Document
...
This document is the single source of truth for building GigShield.
...
## 1.1 What GigShield Is

GigShield is a parametric income insurance platform for food delivery workers on Zomato and Swiggy, starting in Chennai. It protects workers against income loss caused by external disruptions — heavy rain, extreme heat, severe air pollution, or zone curfews — that make delivery work physically impossible.

**Core promise:** When a disruption occurs, GigShield detects it automatically, computes the worker's true income loss, validates the claim against a fraud engine, and deposits the payout to the worker's UPI account within 60 seconds. The worker does nothing. The system does everything.
...
## 1.2 The Core Domain Problem GigShield Solves
...
A worker stopped at delivery 10 by flooding does not lose just 10 deliveries of base pay. They lose the ₹120 bonus they were 2 orders away from. This is a step-function loss, not a linear one. No existing insurance product in India models this. GigShield is the first.

**SEWA 2023 Failure — Why Our Architecture Is What It Is:**
SEWA ran India's only documented parametric income insurance pilot for gig workers in 2023. It triggered zero payouts during the monsoon season. Root cause: they used a single city-level IMD weather station 8km from the affected zone (Velachery, Chennai). On the days Velachery flooded, that station showed below-threshold rainfall. GigShield queries Open-Meteo at 3 geographic points per zone (centroid + 3km NNE offset + 3km SSW offset) and takes the maximum precipitation reading. This directly fixes SEWA's documented failure mode. This 3-point oversampling is not optional — it is the core architectural decision.
...
GigShield operates four concurrent loops that together form the zero-touch parametric insurance engine.
